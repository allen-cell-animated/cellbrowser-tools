...
class VtkSegmentationReader(uiadaptor):
    ...
    def loadVTK_bSplines(self, filename, objprefix, scale=0.1):
        ...
        for i in range(len(linesList)):
            # s =
            self.helper.spline(objprefix+'_spline'+str(i), linesList[i], close=0, type=c4d.SPLINETYPE_BSPLINE, scene=scn, parent=nr)
    ...
    def initWidget(self, id=None):
        ...
        self.PUSHS = {}
        self.PUSHS["Browse"] = self._addElemt(name="Browse Raw VTK lines",
                                              width=40, height=10,
                                              action=self.browseVTK, type="button")
        self.PUSHS["BrowseMito"] = self._addElemt(name="Browse Mito",
                                                  width=40,height=10,
                                                  action=self.browseMito,
                                                  type="button")
        self.PUSHS["BrowseTub"] = self._addElemt(name="Browse Microtubules",
                                                 width=40,height=10,
                                                 action=self.browseTub,
                                                 type="button")
